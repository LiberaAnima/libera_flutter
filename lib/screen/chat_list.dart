          ...
          children: snapshot.data!.docs.map((document) {
            ...
            if (userData['username'] == null || userData['username']!.isEmpty) {
              ...
              return ListTile(
                title: Text(username),
                onTap: () {
                  String conversationId = document.id; // 仮のconversationId生成ロジック
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) => ChatRoom(conversationId: conversationId, userId: myUserId),
                    ),
                  );
                },
              );
            }
          }).where((element) => element is! Container).toList(), // Container（空のリストアイテム）を除外する
        );
